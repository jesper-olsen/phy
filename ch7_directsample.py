...
parser.add_argument('-ex', type=int, choices=[1, 2, 3], required=True, help='Example number (1: Exponential, 2: Cauchy, 3: Triangular)')
...
match EX: 
    ...
    case _:
        ...
        f = lambda x: theta(2 - np.abs(x)) * (theta(-x) * 0.25 * (2 + x) + theta(x) * 0.25 * (2 - x))
...
plt.hist(samples, bins=100, range=(minX, 10), density=True, alpha=0.75, edgecolor='black', label='Histogram')
